...
class RegressionTest {
    ...
    fun testDecrementingStopsAtMinimum() {
        // Drops per row should not go below 4
        var gameState = gameEngine.startNewGame(GameMode.Normal)
        
        // Fast forward to near minimum
        gameState = gameState.copy(dropsUntilNewRow = 1, baseDropsPerRow = 5, nextDisc = Disc.Numbered(5))
        gameState = gameEngine.dropDisc(gameState, 3)
        assertEquals("Should decrement to 4", 4, gameState.dropsUntilNewRow)
        
        // Try to go below minimum
        gameState = gameState.copy(dropsUntilNewRow = 1, nextDisc = Disc.Numbered(5))
        gameState = gameEngine.dropDisc(gameState, 3)
        assertEquals("Should stay at 4 (minimum)", 4, gameState.dropsUntilNewRow)
        
        // Verify it stays at minimum
        gameState = gameState.copy(dropsUntilNewRow = 1, nextDisc = Disc.Numbered(5))
        gameState = gameEngine.dropDisc(gameState, 3)
        assertEquals("Should still be at 4", 4, gameState.dropsUntilNewRow)
    }
    ...
    fun testNewRowMatchesGenerateAnimationSteps() {
        ...
        val hasNewRowStep = dropResult.steps.any { it.isFirstStepAfterNewRow }
    ...
    fun testContiguousRegionRetrieval() {
        ...
        assertTrue("Should include (5,1)", region.contains(5 to 1))
        assertTrue("Should include (5,2)", region.contains(5 to 2))
        assertTrue("Should include (5,3)", region.contains(5 to 3))
    }
