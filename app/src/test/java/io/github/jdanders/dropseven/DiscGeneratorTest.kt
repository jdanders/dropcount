...
class DiscGeneratorTest {
    ...
    fun testNormalModeGeneratesValidDiscs() {
        ...
        repeat(100) {
            ...
            if (disc is Disc.Numbered) {
                assertTrue("Numbered disc value should be 1-7", disc.value in 1..7)
            }
            
            if (disc is Disc.Solid) {
                assertEquals("Solid disc should start with 0 cracks", 0, disc.cracks)
                assertTrue("Hidden value should be 1-7", disc.hiddenValue in 1..7)
            }
        }
    }
    ...
    fun testNormalModeGeneratesMixOfDiscTypes() {
        ...
        assertTrue("Numbered discs should be roughly 70%", 
            numberedPercentage in 0.55..0.85)
    }
    ...
    fun testChallengeModeGeneratesOnlyNumberedDiscs() {
        ...
        repeat(100) {
            ...
            assertTrue("Value should be 1-7", numbered.value in 1..7)
        }
    }
    ...
    fun testChallengeModeAllDifficulties() {
        for (difficulty in ChallengeDifficulty.entries) {
            val mode = GameMode.Challenge(difficulty, false)
            val generator = DiscGenerator(mode)
    ...
    fun testSequenceModeGeneratesMixedTypes() {
        val generator = DiscGenerator(GameMode.Sequence(42))
    ...
    fun testGenerateNewRowCreatesValidRow() {
        ...
        assertEquals("Row should have 7 positions", 7, row.size)
    ...
    fun testGenerateNewRowIsFullyFilled() {
        ...
        assertEquals("Row should be fully filled", 7, row.size)
    ...
    fun testGeneratorDoesNotCrash() {
        for (difficulty in ChallengeDifficulty.entries) {
            val modes = listOf(
                GameMode.Normal,
                GameMode.Challenge(difficulty, false),
                GameMode.Challenge(difficulty, true),
