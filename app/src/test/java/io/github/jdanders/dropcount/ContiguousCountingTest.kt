...
class ContiguousCountingTest {
    ...
    fun testContiguousRowCount() {
        var gameState = TestUtils.createEmptyGameState()

        // Place: [., disc, disc, disc, ., disc, .]
        // Positions 1-3 form a contiguous group of 3
        gameState = gameState.withDiscs(
            Triple(5, 1, Disc.Numbered(1)),
            Triple(5, 2, Disc.Numbered(1)),
            Triple(5, 3, Disc.Numbered(1)),
            Triple(5, 5, Disc.Numbered(1))
        )
    ...
    fun testMatchWithMixedTypes() {
        ...
        gameState = gameState.setCell(6, 0, Cell.Occupied(Disc.Solid(0, 0, 5)))
    ...
    fun testGetContiguousRegionInRow() {
        ...
        assertTrue(region.contains(GridPosition(Row(5), Col(1))))
        assertTrue(region.contains(GridPosition(Row(5), Col(2))))
        assertTrue(region.contains(GridPosition(Row(5), Col(3))))
        assertFalse(region.contains(GridPosition(Row(5), Col(5))))
    }
    ...
    fun testGetContiguousRegionInColumn() {
        ...
        assertEquals(3, region.size)
        assertTrue(region.contains(GridPosition(Row(4), Col(2))))
        assertTrue(region.contains(GridPosition(Row(5), Col(2))))
        assertTrue(region.contains(GridPosition(Row(6), Col(2))))
        assertFalse(region.contains(GridPosition(Row(2), Col(2))))
    }
